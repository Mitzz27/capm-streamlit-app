...
def plotly_table(dataframe):
    ...
    row_odd_color = "#e6efff"

    fig = go.Figure(data=[go.Table(
        header=dict(
            values=["<b></b>"] + ["</b>" + str(i)[:10] + "<b>" for i in dataframe.columns],
            line_color="#0078ff", fill_color="#0078ff", align='center',
...
def RSI(dataframe, num_period):
    ...
    fig.add_trace(go.Scatter(x=dataframe['Date'], y=[30]*len(dataframe), name='Oversold', line=dict(width=2, color='#79da84', dash='dash'), fill='tonexty'))
    fig.update_layout(yaxis_range=[0,100], height=200, plot_bgcolor='white', paper_bgcolor='#e1efff', margin=dict(l=20,r=0,t=0,b=0), legend=dict(orientation='h', yanchor='top', y=1.02, xanchor='right', x=1))
    return fig
...
def MACD(dataframe, num_period):
    ...
    macd_hist = dataframe['MACD Hist']
    colors = ['red' if val < 0 else 'green' for val in macd_hist]
    fig.add_trace(go.Bar(x=dataframe['Date'], y=macd_hist, name='MACD Hist', marker_color=colors))
